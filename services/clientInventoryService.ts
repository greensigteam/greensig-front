...
import { fetchAllSites, apiFetch } from './api';
...
export async function fetchClientInventoryStats(clientId: number): Promise<InventoryStats> {
  ...
  try {
    ...
    console.warn(
      'Backend inventory stats endpoint not available, falling back to client-side calculation',
    );
...
async function computeInventoryStatsClientSide(structureClientId: number): Promise<InventoryStats> {
  ...
  try {
    ...
    const allSites = await fetchAllSites(); // Force refresh
