...
export const useMeasurementTools = ({
  ...
  useEffect(() => {
    ...
    // Handler nommé — doit rester retirable côté cleanup (sinon chaque toggle
    // isMeasuring empile un listener supplémentaire sur le viewport).
    ...
    viewport.addEventListener('mouseout', mouseOutHandler);

    return () => {
      map.removeInteraction(draw);
      map.un('pointermove', pointerMoveHandler);
      viewport.removeEventListener('mouseout', mouseOutHandler);
      drawInteractionRef.current = null;
      if (helpTooltipOverlayRef.current) {
        map.removeOverlay(helpTooltipOverlayRef.current);
        helpTooltipOverlayRef.current = null;
  ...
  // Final unmount cleanup : retire les overlays de mesure encore attachés
  // (tooltip en cours + overlays statiques accumulés) — sinon MapPage qui
  // monte/démonte plusieurs fois fuit des Overlay OL.
  useEffect(() => {
    return () => {
      const map = mapInstance.current;
      if (!map) return;
