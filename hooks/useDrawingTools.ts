...
export const useDrawingTools = ({
  ...
  const deleteClickHandlerRef = useRef<((evt: any) => void) | null>(null);
  ...
  const calculateMetrics = useCallback((geom: Point | LineString | Polygon): GeometryMetrics => {
    ...
    const y1 = extent[1];
    const y3 = extent[3];
    if (x0 !== undefined && x2 !== undefined && y1 !== undefined && y3 !== undefined) {
      metrics.centroid = {
        lng: (x0 + x2) / 2,
        lat: (y1 + y3) / 2,
      };
  ...
  const setFeatureForEditing = useCallback(
    (geojson: GeoJSONGeometry, featureId: string) => {
    ...
    objects.forEach((obj) => {
      if (obj.geometry) {
        try {
          ...
          logger.error('Error loading feature for editing:', obj.id, e);
        }
      }
    });
  }, []);
  ...
  useEffect(() => {
    ...
    if (editingMode === 'modify') {
      ...
      deleteClickHandlerRef.current = deleteClickHandler;
    }

    return () => {
      cleanupInteractions();
      if (deleteClickHandlerRef.current) {
        map.un('click', deleteClickHandlerRef.current);
        deleteClickHandlerRef.current = null;
      }
      // Reset style
