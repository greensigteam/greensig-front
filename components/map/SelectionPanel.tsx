...
import { X, Trash2, MapPin, Square, SquareDashedMousePointer, Ban } from 'lucide-react';
...
export const SelectionPanel: React.FC<SelectionPanelProps> = ({
  ...
  onDeleteObjects: _onDeleteObjects,
  ...
  const { setEditingMode } = useDrawing();
  ...
  useEffect(() => {
    ...
    planningService
      .getApplicableTypesTaches(uniqueObjectTypes)
      .then((result) => {
        setIsCompatible(result.types_taches.length > 0);
        setApplicableTasksCount(result.types_taches.length);
      })
      .catch((_err) => {
        showToast('Erreur lors de la vérification de la compatibilité', 'error');
  ...
  return (
    ...
    >
            ...
            >
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-sm text-slate-700 truncate">{obj.title}</div>
      ...
      {/*
             ...
             * Note: SUPERVISEUR peut modifier (selon matrice permissions), seul CLIENT est en lecture seule
             *
            {selectedObjects.length > 0 && userRole !== 'CLIENT' && (
      ...
      {/* Actions : ADMIN et SUPERVISEUR peuvent créer des tâches (selon matrice permissions) */}
      {selectedObjects.length > 0 && userRole !== 'CLIENT' && (
        <div className="p-3 border-t border-slate-100 bg-slate-50/50 space-y-2">
          ...
          {!isCompatible && !compatibilityLoading && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-2.5 mb-2">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-xs font-medium text-red-700">Types incompatibles</p>
                  <p className="text-xs text-red-600 mt-0.5">
                    Les types sélectionnés ({uniqueObjectTypes.join(', ')}) n'ont aucune tâche en
                    commun.
                  </p>
                </div>
              </div>
          ...
          {isCompatible &&
            ...
            uniqueObjectTypes.length > 1 &&
            !compatibilityLoading && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-2 mb-2">
                <p className="text-xs text-emerald-700 text-center">
                  ✓ {applicableTasksCount} type{applicableTasksCount > 1 ? 's' : ''} de tâche
                  {applicableTasksCount > 1 ? 's' : ''} applicable
                  {applicableTasksCount > 1 ? 's' : ''} aux {uniqueObjectTypes.length} types
                  sélectionnés
                </p>
              </div>
            )}
