...
import { fetchOperateurById, affecterCompetence } from '../services/usersApi';
...
const CompetenceMatrix: React.FC<CompetenceMatrixProps> = ({
  ...
  onNiveauFilterChange: _onNiveauFilterChange,
  onCategorieFilterChange: _onCategorieFilterChange,
}) => {
  const { showToast } = useToast();
  // Effective edit mode - disabled if read-only
  const effectiveEditMode = isEditMode && !isReadOnly;
  const [operateursDetails, setOperateursDetails] = useState<Map<number, OperateurDetail>>(
    new Map(),
  );
  ...
  const loadOperateursDetails = async () => {
    ...
    try {
      ...
      showToast('Erreur lors du chargement des détails opérateurs', 'error');
    } finally {
      setLoading(false);
    }
  };

  const getOperateurNiveau = (
    operateurId: number,
    competenceId: number,
  ...
  const filteredCompetences = useMemo(() => {
    let filtered = [...competences].sort(
      (a, b) => (a.ordreAffichage || 0) - (b.ordreAffichage || 0),
    );

    if (categorieFilter) {
      filtered = filtered.filter((c) => (c.categorieDisplay || c.categorie) === categorieFilter);
    }

    return filtered;
  }, [competences, categorieFilter]);

  const activeOperateurs = operateurs.filter(
    (o) => o.actif !== false && filteredOperateursIds.includes(o.id),
  );
                      ...
                      return (
                        ...
                        >
                          ...
                          {isUpdating ? (
                            ...
                            >
                              {currentNiveau
                                ? NIVEAU_COMPETENCE_LABELS[currentNiveau]
                                : 'Non défini'}
                            </span>
                          )}
                        </div>
            ...
            <table className="w-full border-collapse min-w-max">
              <thead className="sticky top-0 z-20">
                <tr className="bg-gray-100">
                  ...
                  {filteredCompetences.map((comp) => (
                    ...
                    >
                      <div className="truncate max-w-[140px] font-bold text-gray-800">
                        {comp.nomCompetence}
                      </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {visibleOperateurs.map((op, idx) => (
                  <tr
                    key={op.id}
                    className={`transition-all duration-150 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-emerald-50`}
                  >
                    {/* Sticky operator name column */}
                    <td
                      className={`border-r border-gray-300 px-4 py-3 sticky left-0 z-10 shadow-[2px_0_4px_rgba(0,0,0,0.05)] ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                    >
                        ...
                        >
                          {isUpdating ? (
                            ...
                            <select
                              ...
                              className={`w-full px-2 py-1.5 text-xs rounded-lg border-2 cursor-pointer focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all font-medium shadow-sm ${
                                currentNiveau
                            ...
                            <div className="flex items-center justify-center">
                              ...
                              >
                                ...
                                <span>
                                  {currentNiveau ? NIVEAU_COMPETENCE_LABELS[currentNiveau] : '--'}
                                </span>
                              </span>
                            </div>
                          )}
        ...
        {activeOperateurs.length > itemsPerPageLocal && (
          <div className="px-4 py-2 border-t border-gray-200 bg-white text-sm flex items-center justify-between">
            <div className="text-gray-600">
              Affichage {startIndex + 1} - {Math.min(endIndex, activeOperateurs.length)} sur{' '}
              {activeOperateurs.length} opérateurs
            </div>
            <div className="flex items-center gap-2">
              <button
