...
import { MapPin, Minus, Pentagon, Check, Eye, EyeOff, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-50 rounded-lg p-4">
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <div className="text-2xl font-bold text-gray-900">{data.feature_count}</div>
          ...
          <div>
            <div className="text-2xl font-bold text-gray-900">{data.geometry_types.length}</div>
        ...
        {data.geometry_types.map((type) => (
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${
              filterGeomType === type ? 'ring-2 ring-blue-500 ring-offset-1' : ''
      ...
      <div className="flex items-center justify-between border-b pb-2">
        <div className="flex items-center gap-2">
          <button onClick={selectAllVisible} className="text-sm text-blue-600 hover:underline">
            Tout sélectionner
          </button>
          <span className="text-gray-300">|</span>
          <button onClick={deselectAllVisible} className="text-sm text-gray-600 hover:underline">
            Tout désélectionner
          </button>
        </div>
        <button
      ...
      <div className="border rounded-lg divide-y max-h-96 overflow-y-auto">
        {filteredFeatures.length === 0 ? (
          <div className="p-8 text-center text-gray-500">Aucune feature à afficher</div>
        ) : (
              ...
              >
                ...
                <div
                  className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
                    isSelected(feature) ? 'bg-blue-600 border-blue-600' : 'border-gray-300'
                  }`}
                >
                  {isSelected(feature) && <Check className="w-3 h-3 text-white" />}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-gray-900">Feature #{feature.index + 1}</div>
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setExpandedFeature(expandedFeature === feature.index ? null : feature.index);
                  }}
              ...
              {expandedFeature === feature.index && (
                <div className="px-12 pb-3">
                  <div className="bg-gray-100 rounded-lg p-3 text-sm">
                    <div className="font-medium text-gray-700 mb-2">Propriétés</div>
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {data.sample_properties.map((prop) => (
              <span key={prop} className="px-2 py-1 bg-white border rounded text-sm text-gray-700">
                {prop}
              </span>
            ))}
          </div>
