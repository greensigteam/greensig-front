    ...
    <div className="space-y-6">
        ...
        <div className="bg-gray-50 rounded-lg p-4 space-y-4">
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              >
                <Crosshair
                  className={`w-5 h-5 ${autoDetectSite ? 'text-emerald-600' : 'text-gray-500'}`}
                />
              </div>
              <div className="text-left">
                <div
                  className={`font-medium text-sm ${autoDetectSite ? 'text-emerald-700' : 'text-gray-700'}`}
                >
            ...
            >
              ...
              >
                <MapPin
                  className={`w-5 h-5 ${!autoDetectSite ? 'text-blue-600' : 'text-gray-500'}`}
                />
              </div>
              <div className="text-left">
                <div
                  className={`font-medium text-sm ${!autoDetectSite ? 'text-blue-700' : 'text-gray-700'}`}
                >
                  Site unique
                </div>
                <div className="text-xs text-gray-500">
            ...
            <div className="flex items-start gap-2 p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-800">
              ...
              <div className="text-sm">
                ...
                <p className="text-emerald-600">
                  Chaque objet sera automatiquement assigné au site dont l'emprise contient sa
                  géométrie. Les objets hors de tout site seront signalés comme erreurs.
                </p>
              </div>
            </div>
          )}
      ...
      {targetType && (
        <div className="bg-gray-50 rounded-lg p-4 space-y-3">
          <label className="block text-sm font-medium text-gray-700">Mode d'import</label>
          <div className="flex gap-3">
            <button
              type="button"
            ...
            >
              ...
              >
                <Plus
                  className={`w-5 h-5 ${importMode === 'create' ? 'text-blue-600' : 'text-gray-500'}`}
                />
              </div>
              <div className="text-left">
                <div
                  className={`font-medium text-sm ${importMode === 'create' ? 'text-blue-700' : 'text-gray-700'}`}
                >
                  Créer tout
                </div>
                <div className="text-xs text-gray-500">
            ...
            >
              ...
              >
                <SkipForward
                  className={`w-5 h-5 ${importMode === 'skip_duplicates' ? 'text-amber-600' : 'text-gray-500'}`}
                />
              </div>
              <div className="text-left">
                <div
                  className={`font-medium text-sm ${importMode === 'skip_duplicates' ? 'text-amber-700' : 'text-gray-700'}`}
                >
            ...
            >
              <div
                className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                  importMode === 'replace' ? 'bg-green-100' : 'bg-gray-100'
                }`}
              >
                <RefreshCw
                  className={`w-5 h-5 ${importMode === 'replace' ? 'text-green-600' : 'text-gray-500'}`}
                />
              </div>
              <div className="text-left">
                <div
                  className={`font-medium text-sm ${importMode === 'replace' ? 'text-green-700' : 'text-gray-700'}`}
                >
                  Remplacer les doublons
                </div>
                <div className="text-xs text-gray-500">
                  Les objets existants seront mis à jour avec les nouvelles données
                </div>
              </div>
            </button>
          </div>
      ...
      {selectedTypeInfo && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-medium text-gray-900">Mapping des attributs</h3>
            <button onClick={autoSuggestMapping} className="text-sm text-blue-600 hover:underline">
          ...
          <div className="border rounded-lg divide-y">
            ...
            {selectedTypeInfo.fields.map((field) => (
              <div key={field.name} className="flex items-center gap-4 px-4 py-3">
                {/* Source Selector */}
                <div className="w-1/3">
                  <PremiumSelect
                ...
                <div className="w-8 flex justify-center">
                  <ArrowRight
                    className={`w-4 h-4 ${
                      mapping[field.name] ? 'text-green-600' : 'text-gray-300'
                    }`}
                  />
                </div>
          ...
          <div className="flex items-start gap-2 text-sm text-gray-500 bg-blue-50 rounded-lg p-3">
            ...
            <div>
              ...
              <p>
                Les attributs non mappés seront laissés vides. Les champs marqués d'un{' '}
                <span className="text-red-500">*</span> sont recommandés mais pas obligatoires - une
                valeur par défaut sera générée.
              </p>
            </div>
          </div>
        </div>
