...
export default function ValidationResults({ result, onRetry, importMode }: ValidationResultsProps) {
  ...
  const hasAutoDetectedSites = result.features.some((f) => f.detected_site_name !== undefined);
  ...
  const siteGroups = useMemo(() => {
    ...
    result.features.forEach((f) => {
      if (f.is_valid) {
        const siteName = f.detected_site_name || 'Site inconnu';
        if (!groups[siteName]) {
    ...
    <div className="space-y-6">
      ...
      <div className={`grid gap-4 ${existingCount > 0 ? 'grid-cols-4' : 'grid-cols-3'}`}>
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
          ...
          <div className="text-2xl font-bold text-green-700">{result.valid_count}</div>
        ...
        {existingCount > 0 &&
          (importMode === 'replace' ? (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
              ...
              <div className="text-2xl font-bold text-green-700">{existingCount}</div>
              <div className="text-sm text-green-600">Seront remplacés</div>
            </div>
          ) : (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-center">
              <SkipForward className="w-8 h-8 mx-auto text-amber-600 mb-2" />
              <div className="text-2xl font-bold text-amber-700">{existingCount}</div>
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
          ...
          <div className="text-2xl font-bold text-red-700">{result.invalid_count}</div>
        ...
        <div className="flex items-start gap-3 p-4 bg-orange-50 border border-orange-200 rounded-lg">
          ...
          <div>
            ...
            <p className="text-sm text-orange-600 mt-1">
              Ces objets ne sont contenus dans aucun site existant et seront automatiquement exclus
              de l'import.
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {siteGroups.map((group, i) => (
              ...
              >
                ...
                <span className="text-sm text-gray-700 truncate flex-1">{group.siteName}</span>
                <span className="text-sm font-medium text-emerald-600">{group.count}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex items-start gap-3 p-4 bg-green-50 border border-green-200 rounded-lg">
            ...
            <div>
              <p className="font-medium text-green-800">
                {existingCount} objet(s) existent déjà et seront mis à jour
              </p>
              <p className="text-sm text-green-600 mt-1">
                {result.valid_count} nouveau(x) objet(s) seront créés, {existingCount} seront
                remplacés.
              </p>
            </div>
          </div>
            ...
            <div className="divide-y divide-amber-100 max-h-48 overflow-y-auto">
              {existingFeatures.map((feature) => (
                <div key={feature.index} className="px-4 py-2 text-sm flex items-center gap-2">
                  ...
                  <span className="font-medium text-amber-700">Feature #{feature.index + 1}:</span>
            ...
            <div className="divide-y divide-red-100 max-h-48 overflow-y-auto">
              {result.errors.map((error, i) => (
                <div key={i} className="px-4 py-2 text-sm">
                  <span className="font-medium text-red-700">Feature #{error.index + 1}:</span>{' '}
                  <span className="text-red-600">{error.message}</span>
                  <span className="text-red-400 text-xs ml-2">[{error.code}]</span>
                </div>
              ))}
            </div>
          )}
          ...
          {showWarnings && (
            <div className="divide-y divide-yellow-100 max-h-48 overflow-y-auto">
              {result.warnings.map((warning, i) => (
                <div key={i} className="px-4 py-2 text-sm">
                  <span className="font-medium text-yellow-700">Feature #{warning.index + 1}:</span>{' '}
                  <span className="text-yellow-600">{warning.message}</span>
                  <span className="text-yellow-400 text-xs ml-2">[{warning.code}]</span>
                </div>
              ))}
            </div>
          )}
            ...
            <div key={feature.index}>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <span className="font-medium">Feature #{feature.index + 1}</span>
                  <span className="text-gray-500 text-sm ml-2">({feature.geometry_type})</span>
              ...
              {expandedFeatures.has(feature.index) && (
                <div className="px-11 pb-3">
                  <div className="bg-gray-50 rounded p-2 text-sm">
                    <div className="font-medium text-gray-600 mb-1">Propriétés mappées:</div>
                    <div className="grid grid-cols-2 gap-1">
                      {Object.entries(feature.mapped_properties).map(([key, value]) => (
                        <div key={key}>
        ...
        <div className="flex items-center gap-3 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <div className="flex-1">
            <p className="text-sm text-blue-800">
              <strong>{result.invalid_count} objet(s)</strong> seront automatiquement exclus de
              l'import. Seuls les <strong>{result.valid_count} objet(s) valides</strong> seront
              créés.
            </p>
          </div>
          <button
