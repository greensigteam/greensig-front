...
export default function ImportWizard({ isOpen, onClose, onSuccess }: ImportWizardProps) {
  ...
  const handleValidationStep = async () => {
    ...
    if (!targetType || (requiresSite && !hasSiteConfig)) {
      showToast(
        requiresSite
          ? 'Veuillez sélectionner un site ou activer la détection automatique'
          : "Veuillez sélectionner un type d'objet",
        'error',
      );
      return;
    }
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl mx-4 max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div>
            ...
            <p className="text-sm text-gray-500">GeoJSON, KML, KMZ ou Shapefile (ZIP)</p>
          </div>
          <button
            onClick={handleClose}
              ...
              >
                ...
                {selectedFile ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="font-medium text-gray-900">Glissez-déposez votre fichier ici</p>
          ...
          {currentStep === 'complete' && executeResult && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">
                {executeResult.summary.created} objet(s) créé(s) sur {executeResult.summary.total}
              </p>
              {(executeResult.summary.updated ?? 0) > 0 && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-left max-w-md mx-auto mb-4">
                  <div className="flex items-center gap-2 text-green-800">
                    <RefreshCw className="w-5 h-5" />
                    <span className="font-medium">
                      {executeResult.summary.updated} objet(s) mis à jour
                    </span>
                  </div>
                </div>
              )}
              ...
              {executeResult.summary.failed > 0 && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-left max-w-md mx-auto">
                  <div className="flex items-center gap-2 text-yellow-800 mb-2">
                    <AlertTriangle className="w-5 h-5" />
                    <span className="font-medium">{executeResult.summary.failed} erreur(s)</span>
                  </div>
