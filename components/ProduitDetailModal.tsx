...
import { Package, Calendar, Target, CheckCircle, XCircle } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <DetailCard variant={produit.actif ? 'success' : 'default'}>
          ...
          <p
            className={`text-lg font-semibold ${produit.actif ? 'text-green-600' : 'text-gray-600'}`}
          >
        ...
        <DetailCard variant={produit.est_valide ? 'success' : 'danger'}>
          ...
          <p
            className={`text-lg font-semibold ${produit.est_valide ? 'text-green-600' : 'text-red-600'}`}
          >
      ...
      <DetailSection title="Informations générales">
        {produit.cible && (
          <DetailRow label="Cible" value={produit.cible} icon={<Target className="w-4 h-4" />} />
        )}

        {produit.description && <DetailRow label="Description" value={produit.description} />}
        ...
        <DetailSection title={`Doses recommandées (${produit.doses.length})`}>
          <div className="space-y-2">
            {produit.doses.map((dose) => (
              <div key={dose.id} className="p-3 bg-purple-50 rounded-lg border border-purple-200">
                <div className="flex items-center justify-between mb-1">
                  <span className="font-semibold text-purple-700">
                    {dose.dose_valeur} {dose.dose_unite_produit}/{dose.dose_unite_support}
                  </span>
                </div>
                {dose.contexte && <p className="text-sm text-gray-600">{dose.contexte}</p>}
              </div>
            ))}
          </div>
        </DetailSection>
