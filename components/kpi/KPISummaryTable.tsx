...
import {
    ...
    CheckCircle2, Target, ClipboardList, Clock, TableProperties,
} from 'lucide-react';
...
const KPI_ROW_CONFIG: Record<string, { label: string; color: string; Icon: React.ComponentType<{ className?: string; style?: React.CSSProperties }> }> = {
    ...
    taux_realisation_reclamations: { label: 'Taux réalisation réclamations', color: '#f59e0b', Icon: ClipboardList },
    temps_moyen_traitement: { label: 'Temps moyen traitement réclamations', color: '#8b5cf6', Icon: Clock },
};
...
export function KPISummaryTable({ data, onDetailClick }: KPISummaryTableProps) {
    ...
    const globalRows: { key: string; valeur: number | null; valeur_m1: number | null; evolution: number | null; statut: KPIStatus; unite: string; seuil: number | null }[] = [
        { key: 'respect_planning', ...kpis.respect_planning },
        { key: 'qualite_service', ...kpis.qualite_service },
        { key: 'taux_realisation_reclamations', ...kpis.taux_realisation_reclamations },
    ];
            ...
            <table className="w-full">
                <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/50">
                        <th className="text-left px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Indicateur</th>
                        <th className="text-center px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Statut</th>
                        <th className="text-right px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Valeur</th>
                        <th className="text-right px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider hidden sm:table-cell">M-1</th>
                        <th className="text-center px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider hidden sm:table-cell">Évolution</th>
                        <th className="text-right px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Seuil</th>
                        ...
                        return (
                            <>
                                ...
                                >
                                    <td className="px-5 py-3.5 text-sm font-medium text-slate-800">
                                        <div className="flex items-center gap-2.5">
                                            ...
                                            <span className="flex items-center gap-1.5">
                                                {expandedKPI4 ? <ChevronDown className="w-4 h-4 text-slate-400" /> : <ChevronRight className="w-4 h-4 text-slate-400" />}
                                                {cfg.label}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-5 py-3.5">
                                        <button
                                            onClick={(e) => { e.stopPropagation(); onDetailClick('temps_moyen_traitement'); }}
                                            className="p-1.5 text-slate-300 hover:text-emerald-600 opacity-0 group-hover:opacity-100 transition-all rounded-lg hover:bg-emerald-50"
                                            title="Voir détail"
                                        >
                                            <Info className="w-4 h-4" />
                                ...
                                {expandedKPI4 && kpis.temps_moyen_traitement.par_type.map((item) => (
