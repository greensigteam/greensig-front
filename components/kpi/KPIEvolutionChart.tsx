...
const KPI_CONFIGS = [
    ...
    { key: 'taux_realisation_reclamations', label: 'Réalisation réclamations', color: '#f59e0b', seuil: null },
...
export function KPIEvolutionChart({ data }: KPIEvolutionChartProps) {
    ...
    const [visibleKeys, setVisibleKeys] = useState<Set<string>>(
        new Set(['respect_planning', 'qualite_service', 'taux_realisation_reclamations'])
    );
    ...
    const hasPercentage = ['respect_planning', 'qualite_service', 'taux_realisation_reclamations']
        .some((k) => visibleKeys.has(k));
    const hasHours = ['temps_moyen_traitement', 'temps_total_heures']
        .some((k) => visibleKeys.has(k));
                ...
                <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
                    <button
                        ...
                        className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${
                            chartMode === 'line' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:text-slate-700'
                        }`}
                    >
                        Courbe
                    </button>
                    <button
                        ...
                        className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${
                            chartMode === 'bar' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:text-slate-700'
                        }`}
                    >
                        Barres
                    </button>
                    ...
                    {chartMode === 'line' ? (
                        <LineChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                            ...
                            {visibleKeys.has('respect_planning') && (
                                <ReferenceLine yAxisId="percent" y={95} stroke="#059669" strokeDasharray="5 5" strokeOpacity={0.5} />
                            )}
                            {visibleKeys.has('qualite_service') && (
                                <ReferenceLine yAxisId="percent" y={95} stroke="#3b82f6" strokeDasharray="5 5" strokeOpacity={0.5} />
                            )}
                            {/* Lines */}
                            {KPI_CONFIGS.filter((c) => visibleKeys.has(c.key)).map((cfg) => {
                                const yId = ['temps_moyen_traitement', 'temps_total_heures'].includes(cfg.key) ? 'hours' : 'percent';
                                return (
                                    <Line
                                        key={cfg.key}
                                        yAxisId={yId}
