...
import {
    X, CheckCircle2, Target, ClipboardList, Clock, MapPin,
} from 'lucide-react';
import {
...
const KPI_MODAL_CONFIG: Record<string, {
    title: string;
    subtitle: string;
    color: string;
    ...
    taux_realisation_reclamations: {
        title: 'Taux de réalisation des réclamations',
        subtitle: 'Détail et ventilation',
        color: '#f59e0b',
                ...
                <div className="p-6 overflow-y-auto max-h-[calc(85vh-88px)] space-y-5">
                    ...
                    {kpiKey === 'respect_planning' && (
                        <DetailGlobal
                            ...
                            labels={{
                                total_terminees: 'Tâches terminées',
                                dans_delais: 'Dans les délais (≤7j)',
                                en_retard: 'En retard (>7j)',
                            }}
                        />
                    )}
                    ...
                    {kpiKey === 'taux_realisation_reclamations' && (
                        <DetailGlobal
                            details={kpis.taux_realisation_reclamations.details}
                            labels={{
                        ...
                        <>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="p-4 bg-gradient-to-br from-slate-50 to-white rounded-xl border border-slate-100">
                                    ...
                                    <div className="text-xs text-slate-600 font-medium mt-1">Réclamations clôturées</div>
                                </div>
                            </div>

                            {kpis.temps_moyen_traitement.par_type.length > 0 && (
                                <>
                                    <h4 className="text-sm font-semibold text-slate-700 mt-4">Par type de réclamation</h4>
                    ...
                    {kpiKey === 'temps_realisation_tache' && (
                        <>
                            {kpis.temps_realisation_tache.length > 0 ? (
                                    ...
                                    <table className="w-full text-sm">
                                        <thead>
                                            <tr className="border-b border-slate-200">
                                                <th className="text-left py-3 px-4 text-slate-500 font-semibold text-xs uppercase tracking-wider">Type de tâche</th>
                                                <th className="text-left py-3 px-4 text-slate-500 font-semibold text-xs uppercase tracking-wider">Site</th>
                                                <th className="text-right py-3 px-4 text-slate-500 font-semibold text-xs uppercase tracking-wider">Heures</th>
                                                <th className="text-right py-3 px-4 text-slate-500 font-semibold text-xs uppercase tracking-wider">Interventions</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-50">
                                            {kpis.temps_realisation_tache.map((entry, i) => (
                                                <tr key={i} className={`hover:bg-emerald-50/30 transition-colors ${i % 2 === 1 ? 'bg-slate-50/50' : ''}`}>
                                                    <td className="py-3 px-4 text-slate-800 font-medium">{entry.type_tache}</td>
                                                    <td className="py-3 px-4 text-slate-600">{entry.site_nom}</td>
                                                    <td className="py-3 px-4 text-right font-bold text-slate-800">{entry.heures}h</td>
                                                    <td className="py-3 px-4 text-right text-slate-500">{entry.nb_interventions}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                ...
                                <p className="text-slate-400 text-sm text-center py-8">Aucune donnée pour ce mois</p>
                            )}
                        </>
                    )}
    ...
    return (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {Object.entries(labels).map(([key, label]) => {
                ...
                return (
                    <div key={key} className="p-4 bg-gradient-to-br from-slate-50 to-white rounded-xl border border-slate-100">
