...
import {
    AlertCircle, AlertTriangle, Loader2, Clock, MapPin, BarChart3,
    TrendingUp, TrendingDown, Minus, CheckCircle2, Target, ClipboardList,
} from 'lucide-react';
import {
...
const KPI_CARD_CONFIGS = [
    ...
    {
        key: 'taux_realisation_reclamations',
        label: 'Réalisation réclamations',
        color: '#f59e0b',
...
export function KPITab() {
    ...
    const { data: historique, isLoading: historiqueLoading } = useKPIHistorique(
        filters.siteId, 6, { enabled: true }
    );
        ...
        return (
            <div className="p-6 md:p-8 space-y-6">
                ...
                <div className="bg-red-50 border border-red-200 rounded-xl p-8 text-center">
                    ...
                    <p className="text-red-600 text-sm">{(error as Error)?.message || 'Une erreur est survenue'}</p>
                </div>
            </div>
        );
    }
                            ...
                            {hasProgress && (
                                <div className="mb-3">
                                    <div className="flex justify-between text-xs text-slate-500 mb-1">
                                        <span>Seuil: {card.seuil}{card.unite}</span>
                                        <span style={{ color: progressPct >= 100 ? '#059669' : undefined }} className={progressPct >= 100 ? 'font-medium' : ''}>
                            ...
                            {!hasProgress && (
                                <div className="mb-3 flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-slate-500">
                                        {card.statut === 'vert' ? 'Bon' : card.statut === 'orange' ? 'Attention' : card.statut === 'rouge' ? 'Critique' : 'N/A'}
                                    </span>
                                </div>
                            )}
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                    <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-slate-400">
                            Total: <strong className="text-slate-700">{kpis.temps_total_par_site.total_heures}h</strong>
                        </span>
                    </div>
