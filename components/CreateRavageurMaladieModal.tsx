...
import { Bug, Power, FileText, AlertTriangle, Target } from 'lucide-react';
import {
...
interface CreateRavageurMaladieModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (data: RavageurMaladieCreate) => Promise<void>;
    produits: ProduitList[];
}

const CreateRavageurMaladieModal: React.FC<CreateRavageurMaladieModalProps> = ({
    isOpen,
    onClose,
    onSubmit,
    produits
    ...
    const handleProductToggle = (productId: number) => {
        ...
        if (current.includes(productId)) {
            handleChange('produits_recommandes', current.filter(id => id !== productId));
        } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error('Erreur creation ravageur/maladie:', err);
            const errorMessage = err.message || 'Erreur lors de la creation';
    ...
    const categorieOptions = Object.entries(CATEGORIE_RAVAGEUR_MALADIE_LABELS).map(([value, label]) => ({
        value,
        label
    }));
    ...
    return (
        <FormModal
            ...
            title="Nouveau ravageur / maladie"
            subtitle="Ajouter un ravageur ou une maladie au catalogue"
        ...
        >
            ...
            <FormSection
                title="Diagnostic"
                description="Symptomes et parties affectees"
            >
                <FormGrid columns={1}>
                    <PremiumTextarea
            ...
            <FormSection
                title="Statut"
                description="Disponibilite dans le systeme"
            >
                <div
...
export default CreateRavageurMaladieModal;
