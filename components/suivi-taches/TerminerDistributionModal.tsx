...
import { X, CheckCircle, Clock, Calendar, AlertCircle } from 'lucide-react';
...
export const TerminerDistributionModal: React.FC<TerminerDistributionModalProps> = ({
    ...
    const calculateHours = (): number | null => {
        ...
        const [h1, m1] = heureDebutReelle.split(':').map(Number);
        const [h2, m2] = heureFinReelle.split(':').map(Number);
                        ...
                        <p className="text-sm text-slate-600">
                            ...
                            {distribution.heure_debut && distribution.heure_fin && (
                                <span className="text-slate-500 ml-1">
                                    ({distribution.heure_debut.substring(0, 5)} - {distribution.heure_fin.substring(0, 5)})
                                </span>
                            )}
                        </p>
                    </div>
