...
import { X, Clock, ChevronDown, Search, Filter, AlertTriangle, Users, Repeat } from 'lucide-react';
import {
...
const DistributionFiltersComponent: FC<DistributionFiltersProps> = ({
    ...
    const { refs: statusRefs, floatingStyles: statusFloatingStyles, context: statusContext } = useFloating({
        open: isStatutOpen,
    ...
    const { getReferenceProps: getStatusRefProps, getFloatingProps: getStatusFloatingProps } = useInteractions([statusDismiss]);

    // Floating UI pour popover "Plus de filtres"
    const { refs: moreRefs, floatingStyles: moreFloatingStyles, context: moreContext } = useFloating({
        open: isMoreOpen,
    ...
    const { getReferenceProps: getMoreRefProps, getFloatingProps: getMoreFloatingProps } = useInteractions([moreDismiss]);
        ...
        <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide py-1 flex-wrap">
            ...
            <div className="shrink-0">
                ...
                >
                    ...
                    Statut {filters.status_in && filters.status_in.length > 0 && `(${filters.status_in.length})`}
            ...
            {equipes.length > 0 && (
                <div className="relative shrink-0">
                    ...
                    <select
                        ...
                        onChange={(e) => handleEquipeChange(e.target.value ? Number(e.target.value) : undefined)}
                        disabled={disabled}
            ...
            <div className="flex items-center gap-1 shrink-0">
                ...
                <button
                    onClick={() => handleShortcutChange('est_report', filters.est_report === true ? undefined : true)}
                    disabled={disabled}
                    className={`
                        ...
                        >
                            ...
                            <label className="flex items-center gap-3 cursor-pointer">
                                <input
                                    ...
                                    onChange={(e) => handleShortcutChange('termine', e.target.checked ? true : undefined)}
                                    className="w-4 h-4 text-emerald-600 border-slate-300 rounded focus:ring-emerald-500"
                                />
