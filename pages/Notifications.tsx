...
import {
  ...
  type Notification as ContextNotification,
...
export default function NotificationsPage({ user }: NotificationsPageProps) {
  ...
  // Admin: Onglet actif persisté dans l'URL
  const activeTab = (searchParams.get('tab') as 'mine' | 'all' | 'actions') || 'mine';
  ...
  // Filtres — tous persistés dans l'URL pour survivre aux navigations
  const filterType = searchParams.get('type') || '';
  const filterStatus = searchParams.get('lu') || '';
  const filterPriority = searchParams.get('priorite') || '';
  ...
  // Les filtres sont maintenant directement dans l'URL — pas besoin de sync séparée
  ...
  // Handlers
  const handleNotificationClick = async (notification: ContextNotification) => {
    // Marquer comme lu
    if (!notification.lu) {
          ...
          <div className="flex gap-2">
            ...
            {isAdmin && (
              ...
              >
                ...
                <span className="ml-1 px-1.5 py-0.5 text-[10px] bg-purple-100 text-purple-700 rounded-full font-bold">
                  ADMIN
                </span>
              </button>
            )}
          ...
          <div className="mt-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
            <div className="flex flex-wrap gap-4">
              ...
              <div className="min-w-[150px]">
                ...
                <select
                  value={filterStatus}
                  onChange={(e) => setFilter('lu', e.target.value)}
                  className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                >
