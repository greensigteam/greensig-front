...
import {
    Edit2, Trash2, FileText, Search, RefreshCw, Download, AlertTriangle
} from 'lucide-react';
...
import {
    TypeTache, TypeTacheCreate, RatioProductivite
} from '../types/planning';
...
const TypesTachesConfig: React.FC<TypesTachesConfigProps> = ({ triggerCreate }) => {
    ...
    const handleEdit = useCallback((typeTache: TypeTacheWithRatiosCount) => {
    ...
    const handleSave = async (data: TypeTacheCreate, compatibleObjects: string[]) => {
        ...
        await planningService.saveTypeTacheWithRatios(
            data,
            ratiosToCreate,
            selectedTypeTache?.id
        );
    ...
    const handleBulkDelete = async () => {
        ...
        for (const idStr of selectedIds) {
            ...
            try {
                ...
                console.error(`Erreur suppression type ${id}:`, err);
                errorCount++;
            }
        }
