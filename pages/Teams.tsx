...
const StatutOperateurBadge: React.FC<{ statut: StatutOperateur }> = ({ statut }) => {
  const colors = STATUT_OPERATEUR_COLORS[statut];
  return (
    <span className={`px-2 py-1 rounded-full text-xs font-medium ${colors.bg} ${colors.text}`}>
      {STATUT_OPERATEUR_LABELS[statut]}
    </span>
  );
};

const StatutEquipeBadge: React.FC<{ statut: StatutEquipe }> = ({ statut }) => {
  const colors = STATUT_EQUIPE_COLORS[statut];
  return (
    <span className={`px-2 py-1 rounded-full text-xs font-medium ${colors.bg} ${colors.text}`}>
      {STATUT_EQUIPE_LABELS[statut]}
    </span>
  );
};

const StatutAbsenceBadge: React.FC<{ statut: StatutAbsence }> = ({ statut }) => {
  const colors = STATUT_ABSENCE_COLORS[statut];
  return (
    <span className={`px-2 py-1 rounded-full text-xs font-medium ${colors.bg} ${colors.text}`}>
      {STATUT_ABSENCE_LABELS[statut]}
    </span>
  );
};

const NiveauCompetenceBadge: React.FC<{ niveau: NiveauCompetence }> = ({ niveau }) => {
  const colors = NIVEAU_COMPETENCE_COLORS[niveau];
  return (
    <span className={`px-2 py-1 rounded-full text-xs font-medium ${colors.bg} ${colors.text}`}>
...
const CreateTeamModal: React.FC<CreateTeamModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.chefEquipe === 0) {
      setError("Veuillez selectionner un chef d'equipe");
      return;
    }
    ...
    try {
      const equipe = await createEquipe({
        ...
        chefEquipe: formData.chefEquipe,
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="p-6 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Chef d'equipe <span className="text-red-500">*</span>
              </label>
              <select
                required
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-gray-900">
                {operateur.estChefEquipe ? "Chef d'equipe" : 'Operateur'}
...
type TabType = 'equipes' | 'operateurs' | 'absences';
...
const Teams: React.FC = () => {
  ...
  const filteredAbsences = absences.filter(a =>
    a.operateurNom.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const absencesColumns: Column<Absence>[] = [
    { key: 'operateurNom', label: 'Operateur' },
    {
      ...
      label: 'Type',
      render: (a) => {
        const colors = TYPE_ABSENCE_COLORS[a.typeAbsence];
        return (
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${colors.bg} ${colors.text}`}>
