    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-start gap-2">
          ...
          <div className="text-sm text-amber-800">
            <p className="font-medium">Impact sur la planification</p>
            <p className="text-amber-700 mt-1">
              Les jours fériés actifs seront automatiquement skippés lors de la génération de tâches récurrentes.
            </p>
          </div>
        </div>
      </div>
      ...
      {loading && joursFeries.length === 0 ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-slate-700 mb-2">Aucun jour férié configuré</h3>
          <p className="text-slate-500 mb-4">
            Commencez par importer les jours fériés du Maroc ou ajoutez-les manuellement.
          </p>
          <button
            onClick={() => handleImporterMaroc(anneeActuelle)}
            className="mx-auto flex items-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors font-medium"
          >
            <Download className="w-5 h-5" />
            Importer les jours fériés {anneeActuelle}
          </button>
        </div>
      ) : (
        <div className="space-y-6">
          {annees.map(annee => (
            <div key={annee} className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
